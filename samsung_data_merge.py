...
df_standard = pd.read_csv ('./sample_csv/fd_origin.csv', encoding = 'cp949')
df_to_add = pd.read_csv ('./sample_csv/fd_2018_5.csv', encoding = 'cp949')
...
for i in range(len(df_samsung_0)):
    ...
    for j in range(len(df_standard_list)):
        ...
        if origin_index == match_index and df_samsung_0.iloc[i,4] != 'SCF':  df_dummy.iloc[i,8] = new_index
        elif origin_index == match_index and df_samsung_0.iloc[i,4] == 'SCF':  df_dummy.iloc[i,8] = refer_index

# result key is "df_dummy"
df_dummy.to_excel("./samsung_data_merge.xlsx")